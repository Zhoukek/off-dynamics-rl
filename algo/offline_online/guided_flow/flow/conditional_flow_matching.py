...
class ExactOptimalTransportConditionalFlowMatcher(ConditionalFlowMatcher):
    """Uses OTPlanSampler (now JAX OTT based)"""

    def __init__(self, sigma: Union[float, int] = 0.0, seed: int = 42): # Added seed for JAX RNG
        r"""Initialize with JAX OTT sampler."""
        super().__init__(sigma)
        # Initialize the JAX OTT based sampler
        self.ot_sampler = OTPlanSampler(method="sinkhorn") # Will use Sinkhorn approx by default now
        # --- Initialize JAX PRNG Key ---
        self.key = jax.random.PRNGKey(seed)

    def sample_location_and_conditional_flow(self, x0, x1, t=None, return_noise=False):
        r"""Uses JAX OTT sampler; requires key management."""
        # --- Split JAX Key ---
        self.key, subkey = jax.random.split(self.key)

        # Sample (x0, x1) coupling using JAX OTT; returns PyTorch tensors
        x0_coupled, x1_coupled = self.ot_sampler.sample_plan(subkey, x0, x1)

        # Call parent method with the OT-sampled PyTorch tensors
        return super().sample_location_and_conditional_flow(x0_coupled, x1_coupled, t, return_noise)

    def guided_sample_location_and_conditional_flow(
        self, x0, x1, y0=None, y1=None, t=None, return_noise=False
    ):
        r"""Uses JAX OTT sampler with labels; requires key management."""
         # --- Split JAX Key ---
        self.key, subkey = jax.random.split(self.key)

        # Sample coupling with labels using JAX OTT; returns PyTorch tensors
        x0_c, x1_c, y0_c, y1_c = self.ot_sampler.sample_plan_with_labels(
            subkey, x0, x1, y0, y1
        )

        # Call parent method with the OT-sampled PyTorch tensors
        if return_noise:
            t, xt, ut, eps = super().sample_location_and_conditional_flow(x0_c, x1_c, t, return_noise)
            return t, xt, ut, y0_c, y1_c, eps # Return sampled labels
        else:
            t, xt, ut = super().sample_location_and_conditional_flow(x0_c, x1_c, t, return_noise)
            return t, xt, ut, y0_c, y1_c # Return sampled labels
